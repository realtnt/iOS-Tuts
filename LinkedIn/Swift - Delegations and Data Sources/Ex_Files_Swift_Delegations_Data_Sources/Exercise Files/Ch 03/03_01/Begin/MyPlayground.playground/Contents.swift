...
class Colors{
    var dataSource: ColorChooserDataSource?
    var numberOfColors: Int { dataSource?.numberOfColors() ?? 0 }
    var colorIndex = 0

    func name(_ index:Int) -> String{
//        return colorName[index % colorName.count]
        colorIndex = index % numberOfColors
        return dataSource?.nameOfColor(for: colorIndex) ?? ""
    }
    func color(_ index:Int) -> UIColor{
//        return color[index % color.count]
        colorIndex = index % numberOfColors
        return dataSource?.colorOfColor(for: colorIndex) ?? UIColor.black
    }
}
