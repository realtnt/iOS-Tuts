...
struct Classroom {
    ...
    func curveGrades() -> Classroom {
        let difference = 100 - getHighestGrade()!
        var newStudents: [Student] = []
        for student in self.students {
            newStudents.append(Student(
                                    firstName: student.firstName,
                                    lastName: student.lastName,
                                    grade: student.grade + difference)
            )
        }
        return Classroom(className: self.className, students: newStudents.sorted { $0.grade > $1.grade })
    }
}
